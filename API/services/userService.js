...
async function loginUser(req, res) {
    try {
        ...
        if (result.data.rows.length > 0) {
            ...
            const comparePassword = async function(password, hash) {
                try {
                    if (await bcrypt.compare(password, hash)) {
                        ...
                        jwt.sign({ user: data }, config.crypt.secret, { expiresIn: config.crypt.token_expire }, (err, token) => {
                            helper.sendResponse(res, 200, { token: token }, null);
                        });
                    } else {
                        helper.sendResponse(res, 201, null, "Credentials are invalid.");
                    }
                } catch (err) {
                    console.log(err);
                }
            ...
            helper.sendResponse(res, 201, null, "Credentials doesn't exist");
        }
    } catch (err) {
...
async function registerUser(req, res) {
    try {
        ...
        var result = await config.executeQuery(`CALL user_Registration("${req.body.firstName}", "${req.body.lastName}", "${req.body.phoneNo}", "${req.body.gender}", "${req.body.email}", "${encryptedPassword}", "${code}", '${req.body.role}', @output); SELECT @output as output;`);
        ...
        if (output.code == 200) {
            ...
            mail.sendEmail(req, mail.getEmailTemplate(1, { code: req.body.email + '+' + code }).subject, mail.getEmailTemplate(1, { code: req.body.email + '+' + code }).body);
        }
...
async function updateUser(req, res) {
    try {
        var result = await config.executeQuery(`CALL user_updateUserDetails("${req.body.firstName}", "${req.body.lastName}", "${req.body.phoneNo}", "${req.body.gender}", "${req.body.id}", @output); SELECT @output as output;`);
...
async function forgotUserPassword(req, res) {
    try {
        ...
        mail.sendEmail(req, mail.getEmailTemplate(2, { code: code }).subject, mail.getEmailTemplate(2, { code: code }).body);
    ...
    try {
        ...
        if (result.data.rows.length > 0) {
            ...
            if (result.data.rows.affectedRows == 1) {
                ...
                helper.sendResponse(res, 201, null, null);
            }
        } else {
            helper.sendResponse(res, 201, null, "Credentials doesn't exist");
        }
...
async function getUserDetails(id, res) {
    try {
        ...
        if (result.data.rows.length > 0) {
            ...
            helper.sendResponse(res, 201, null, "User doesn't exist.");
        }
    } catch (err) {
        helper.sendResponse(res, 500, null, err);
    }
